...
const Content = ({ setMessage }) => {
  ...
  const getItemsFromDb = () => {
    const itemsObject = getItemsFetch();
    processRetrievedItemsObject(itemsObject);
  };

  const addNewItemToDb = (newItem) => {
    const itemsObject = addItemFetch(newItem);
    processRetrievedItemsObject(itemsObject);
  };

  const updateItemsAtDb = (updItemsObj) => {
    const itemsObject = updateItemsFetch(updItemsObj);
    processRetrievedItemsObject(itemsObject);
  };

  const deleteItemFromDb = (itemId) => {
    const itemsObject = deleteItemFetch(itemId);
    processRetrievedItemsObject(itemsObject);
  };
